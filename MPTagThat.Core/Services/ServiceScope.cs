...
namespace MPTagThat.Core
{
  ...
  /// </remarks>
  /// <example>
  ///   This example creates a new ServiceScope and adds its own implementation
  ...
  /// </example>
  /// <example>
  ///   This is an example of how to pass the current ServiceScope to a
  ///   timer thread.
  ///   <code>
  ...
  public sealed class ServiceScope : IDisposable
  {
      ...
      get
      {
        if (current == null)
        {
          if (global == null)
          {
            ...
            current.services.Add(typeof (IThemeManager),
                                 new ServiceCreatorCallback<IThemeManager>(ThemeManagerRequested));
            current.services.Add(typeof (ISettingsManager),
                                 new ServiceCreatorCallback<ISettingsManager>(SettingsManagerRequested));
            current.services.Add(typeof (ILocalisation),
                                 new ServiceCreatorCallback<ILocalisation>(LocalisationRequested));
            current.services.Add(typeof (IMessageBroker),
                                 new ServiceCreatorCallback<IMessageBroker>(MessageBrokerRequested));
          }
          current = global;
        }
        return current;
    ...
    private static IThemeManager ThemeManagerRequested(ServiceScope scope)
    {
      IThemeManager thememanager = new NoThemeManager();
      scope.ReplaceService(thememanager);
      return thememanager;
    }
    ...
    {
      Current.ReplaceService(service);
    }
    ...
    private T GetService<T>(bool throwIfNotFound)
    {
      ...
      if (oldInstance == null)
      {
        if (throwIfNotFound)
        {
          // Designer has problems displaying elements using the service
