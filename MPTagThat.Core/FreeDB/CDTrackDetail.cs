...
namespace MPTagThat.Core.Freedb
{
                         ...
                         int trackNumber, int offset, int duration)
    {
      ...
      m_offset = offset;
      m_duration = duration;
      m_durationString = string.Format("{0}:{1}", (m_duration / 60).ToString().PadLeft(2, '0'), (m_duration % 60).ToString().PadLeft(2, '0'));
    }
    ...
    public string Duration
    {
      get
      {
        return string.Format("{0}:{1}", (m_duration / 60).ToString().PadLeft(2, '0'), (m_duration % 60).ToString().PadLeft(2, '0'));
      }
    }
