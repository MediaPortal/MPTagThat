...
namespace MPTagThat.Core.ShellLib
{
  public class ShellApi
  {
    ...
    public static extern IntPtr ShellExecute(
      ...
      // Pointer to a null-terminated string that specifies the file or object on which 
      // to execute the specified verb.
      [MarshalAs(UnmanagedType.LPStr)] String lpParameters,
      // If the lpFile parameter specifies an executable file, lpParameters is a pointer 
    ...
    #region Nested type: ComboBoxInfo
    ...
    public struct SHELLEXECUTEINFO
    {
      ...
      [MarshalAs(UnmanagedType.LPWStr)] public String lpVerb;
                                                      // String, referred to as a verb, that specifies the action to 

      // be performed. 
      [MarshalAs(UnmanagedType.LPWStr)] public String lpFile;
                                                      // Address of a null-terminated string that specifies the name of 

      // the file or object on which ShellExecuteEx will perform the 
      // action specified by the lpVerb parameter.
      [MarshalAs(UnmanagedType.LPWStr)] public String lpParameters;
                                                      // Address of a null-terminated string that contains the 
    ...
    #region Nested type: STRRETinternal

    [StructLayout(LayoutKind.Explicit)]
    public struct STRRETinternal
    {
      [FieldOffset(0)] public IntPtr pOleStr; // must be freed by caller of GetDisplayNameOf

      [FieldOffset(0)] public IntPtr pStr; // NOT USED

      [FieldOffset(0)] public UInt32 uOffset; // Offset into SHITEMID

      [FieldOffset(0)] public IntPtr cStr; // Buffer to fill in (ANSI)
    }
